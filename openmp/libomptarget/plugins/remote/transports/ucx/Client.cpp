...
ClientManagerTy::ClientManagerTy() {
  ManagerConfigTy Config;
  for (auto &ConnectionConfig : Config.ConnectionConfigs)
    Clients.emplace_back((BaseClientTy *)new ClientTy(ConnectionConfig));
}

ClientTy::ClientTy(ConnectionConfigTy Config) : Interface(Context, Config) {}
...
int32_t ClientTy::getNumberOfDevices() {
  ...
  const auto NumDevices = Serializer.I32(Response);
...
int32_t ClientTy::registerLib(__tgt_bin_desc *Description) {
  ...
  const auto Response = send(MessageKind::RegisterLib,
...
int32_t ClientTy::unregisterLib(__tgt_bin_desc *Description) {
  CLIENT_DBG("Unregistering library %p", Description)

  const auto Response =
      send(MessageKind::UnregisterLib, Serializer.emptyMessage());

  CLIENT_DBG("Unregistered library %p", Description)
  return Serializer.I32(Response);
}
...
int64_t ClientTy::initRequires(int64_t RequiresFlags) {
  ...
  std::call_once(RequiresOnceFlag, [this, &ReturnValue, RequiresFlags]() {
    ...
    ReturnValue = Serializer.I64(Response);
  });
...
int32_t ClientTy::runTargetTeamRegion(int32_t DeviceId, void *TgtEntryPtr,
  ...
  auto Response =
      send(RunTargetTeamsRegion,
           Serializer.TargetTeamRegion(DeviceId, TgtEntryPtr, TgtArgs,
                                       TgtOffsets, ArgNum, TeamNum, ThreadLimit,
                                       LoopTripCount, RemoteEntries[DeviceId]));
