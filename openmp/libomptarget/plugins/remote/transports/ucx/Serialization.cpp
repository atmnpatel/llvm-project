...
BaseMessageTy::BaseMessageTy(size_t Size)
    : MessageSize(Size) {
  Message = new char[Size];
  CurBuffer = Message;
}

BaseMessageTy::BaseMessageTy(std::string_view M)
    : CurBuffer(const_cast<char*>(M.data())) {
  }

void BaseMessageTy::serialize(uintptr_t Value) {
  *(uintptr_t *)CurBuffer = Value;
  CurBuffer += sizeof(Value);
}

void BaseMessageTy::serialize(void *BufferStart, void *BufferEnd) {
...
void BaseMessageTy::serialize(char *String) {
  serialize(String, String + strlen(String));
}

void BaseMessageTy::serialize(__tgt_offload_entry *Entry) {
...
void BaseMessageTy::serialize(__tgt_device_image *Image) {
  auto NumEntries = 0;
...
void *BaseMessageTy::deserializePointer() {
...
void BaseMessageTy::deserialize(void *&BufferStart, void *&BufferEnd) {
  size_t StrSize = 0;
  deserialize(StrSize);
...
void BaseMessageTy::deserialize(char *&String) {
...
void BaseMessageTy::deserialize(__tgt_offload_entry *&Entry) {
...
void BaseMessageTy::deserialize(__tgt_device_image *&Image) {
...
I32::I32(int32_t Value) : BaseMessageTy(sizeof(Value)), Value(Value) {
  serialize(Value);
}

I32::I32(std::string_view MessageBuffer) : BaseMessageTy(MessageBuffer) {
  Value = *((int32_t *)CurBuffer);
}

I64::I64(int64_t Value) : BaseMessageTy(sizeof(Value)) { serialize(Value); }

I64::I64(std::string_view MessageBuffer) : BaseMessageTy(MessageBuffer) {
  std::memcpy(&Value, CurBuffer, sizeof(int64_t));
}

Pointer::Pointer(uintptr_t Value) : BaseMessageTy(sizeof(Value)) {
  serialize(Value);
}

Pointer::Pointer(std::string_view MessageBuffer) : BaseMessageTy(MessageBuffer) {
...
Binary::Binary(int32_t DeviceId, __tgt_device_image *Image)
    : BaseMessageTy(sizeof(DeviceId) + sizeof(Image)) {
  serialize(DeviceId);
  serialize((uintptr_t)Image);
}

Binary::Binary(std::string_view MessageBuffer) : BaseMessageTy(MessageBuffer) {
...
DataDelete::DataDelete(int32_t DeviceId, void *TgtPtr)
    : BaseMessageTy(sizeof(DeviceId) + sizeof(TgtPtr)) {
  serialize(DeviceId);
...
DataSubmit::DataSubmit(int32_t DeviceId, void *TgtPtr, void *HstPtr,
                       int64_t DataSize)
    : BaseMessageTy(sizeof(DeviceId) + sizeof(uintptr_t) + sizeof(DataSize) +
                DataSize) {
  serialize(DeviceId);
  serialize((uintptr_t)TgtPtr);
...
Data::Data(int32_t Value, char *Buffer, size_t DataSize)
    : BaseMessageTy(sizeof(DataSize) + DataSize + sizeof(Value)) {
  serialize(Value);
  serialize(Buffer, Buffer + DataSize);
}

Data::Data(std::string_view MessageBuffer) : BaseMessageTy(MessageBuffer) {
  deserialize(Value);
  DataBuffer = nullptr;
  void *BufferEnd = nullptr;
...
TargetRegion::TargetRegion(int32_t DeviceId, void *TgtEntryPtr, void **TgtArgs,
    ...
    : BaseMessageTy(sizeof(DeviceId) + sizeof(TgtEntryPtr) + sizeof(ArgNum) +
